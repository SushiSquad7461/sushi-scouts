...
import '../sushi_scouts/scoutingcomponent.dart';
  ...
  void updateData(String key) {
    setState(() {
      ...
      if (flagMode || deleteMode) {
        ...
        List<String> splitData = data[key]!.data.split("\n");
        ConfigFileReader reader = ConfigFileReader.instance;
        ScoutingData? editData;

        for (var i in reader.getScoutingMethods()) {
          if (i[0].toUpperCase() == splitData[0][0]) {
            editData = reader.generateNewScoutingData(i);
          }
        }

        if (editData != null) {
          for (int i = 1; i < splitData.length - 1; ++i) {
            editData.pages[editData.pageNames[i - 1]]!
                .fromStringfy((json.decode(splitData[i]))["sections"]);
          }
      ...
      body: Column(
        ...
        children: [
          ...
          editData == null ?
            ...
            ) : ScoutingComponent(scoutingData: editData!,),
          if (editData == null) OpacityFilter(
